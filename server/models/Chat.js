...
chatSchema.methods.makeCounterOffer = function (userId, amount) {
  ...
  const counterMessage = `${
    isBuyer ? "Buyer" : "Seller"
  } counter-offered ₹${amount.toLocaleString()}`;
...
chatSchema.methods.acceptOffer = function (userId) {
  ...
  const acceptMessage = `${
    isBuyer ? "Buyer" : "Seller"
  } accepted the offer of ₹${this.currentOffer.amount.toLocaleString()}`;
...
chatSchema.methods.rejectOffer = function (userId, reason = "") {
  ...
  const isBuyer = userId.toString() === this.buyer.toString();
  const rejectMessage = `${isBuyer ? "Buyer" : "Seller"} rejected the offer${
    reason ? ": " + reason : ""
  }`;
...
chatSchema.methods.isParticipant = function (userId) {
  return (
    userId.toString() === this.buyer.toString() ||
...
chatSchema.methods.toJSON = function () {
  ...
  chat.hasUnreadMessages =
    this.metadata.unreadCount.buyer > 0 || this.metadata.unreadCount.seller > 0;

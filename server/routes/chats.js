...
router.post("/start", auth, async (req, res) => {
  try {
    ...
    console.log(
      `Chat started between ${req.user.email} and ${land.currentOwner.email} for land ${land.landId}`
    );
...
router.get("/my-chats", auth, async (req, res) => {
  try {
    const chats = await Chat.findByUser(req.user._id).sort({
...
router.post("/:chatId/message", auth, async (req, res) => {
  try {
    const { message, messageType = "TEXT", dedupeKey = null } = req.body;
    ...
    // Deduplication: check for existing message with dedupeKey
    if (dedupeKey) {
      const exists = chat.messages.some((m) => m.dedupeKey === dedupeKey);
      if (exists) {
        return res.status(200).json({
          message: "Duplicate message ignored",
          newMessage: chat.messages.find((m) => m.dedupeKey === dedupeKey),
        });
      }
    }
...
router.post("/:chatId/offer", auth, async (req, res) => {
  try {
    ...
    if (!offerAmount || offerAmount <= 0) {
      return res
        .status(400)
        .json({ message: "Valid offer amount is required" });
    }
    ...
    if (!chat.canMakeOffer(req.user._id)) {
      return res
        .status(400)
        .json({ message: "Cannot make offer at this time" });
    }
    ...
    console.log(
      `Offer made in chat ${chat._id}: ₹${offerAmount} by ${req.user.email}`
    );
...
router.post("/:chatId/counter-offer", auth, async (req, res) => {
  try {
    ...
    try {
      ...
      console.log(
        `Counter offer made in chat ${chat._id}: ₹${counterAmount} by ${req.user.email}`
      );
...
router.post("/:chatId/accept", auth, async (req, res) => {
  try {
    ...
    if (!chat.canAcceptOffer(req.user._id)) {
      return res
        .status(400)
        .json({ message: "Cannot accept offer at this time" });
    }
    ...
    try {
      ...
      console.log(
        `Offer accepted in chat ${chat._id}: ₹${chat.currentOffer.amount} by ${req.user.email}`
      );

...
const AdminPanel: React.FC = () => {
  const [activeTab, setActiveTab] = useState<
    "transactions" | "users" | "lands" | "land-transactions"
  >("users");
    ...
    try {
      ...
      switch (activeTab) {
        ...
        case "land-transactions":
          const landTransResponse =
            await apiService.getPendingLandTransactions();
  ...
  const handleReviewLandTransaction = async (
    transactionId: string,
  ...
  const digitalizeLand = async (land: Land) => {
    try {
      setError("");
      setProcessingId(land.id);
      const response = await fetch(`/api/lands/${land.id}/digitalize`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      });
      const result = await response.json();
      if (result.success) {
        alert("Land digitalization initiated successfully");
        await loadData();
      } else {
        setError(result.message || "Failed to initiate digitalization");
      }
    } catch (error: any) {
      setError(error.message || "Failed to initiate digitalization");
    } finally {
      setProcessingId(null);
    }
  };
                ...
                <div className="bg-gray-50 rounded-md p-3 mb-4">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    {user.verificationDocuments?.panCard && (
                      <div>
                        ✓ PAN Card: {user.verificationDocuments.panCard.number}
                      </div>
                    )}
                    {user.verificationDocuments?.aadhaarCard && (
                      <div>
                        ✓ Aadhaar:{" "}
                        {user.verificationDocuments.aadhaarCard.number}
                      </div>
                    )}
                    {user.verificationDocuments?.drivingLicense && (
                      <div>
                        ✓ DL: {user.verificationDocuments.drivingLicense.number}
                      </div>
                    )}
                    {user.verificationDocuments?.passport && (
                      <div>
                        ✓ Passport: {user.verificationDocuments.passport.number}
                      </div>
                    )}
                  </div>
                </div>
          ...
          {allLands.map((land) => (
            ...
            >
              ...
              <div className="space-y-2 text-sm text-gray-600">
                ...
                <div>
                  Digitalized:{" "}
                  {land.digitalDocument.isDigitalized ? "Yes" : "No"}
                </div>
              </div>
            </div>
          ))}
    ...
    <div className="space-y-4">
      {landTransactions.length === 0 ? (
        <div className="text-center py-12">
          ...
          <div className="text-gray-400 text-lg mt-4">
            No pending land transactions
          </div>
          <p className="text-gray-500 mt-2">
            All land transactions have been processed.
          </p>
        </div>
      ) : (
          ...
          >
            <div className="flex items-start justify-between">
              ...
              <div className="flex flex-col space-y-2 ml-4">
                <button
                  onClick={() =>
                    handleReviewLandTransaction(transaction._id, "approve")
                  }

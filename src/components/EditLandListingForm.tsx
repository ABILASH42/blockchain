...
interface EditLandListingFormProps {
  land: Land;
  onClose: () => void;
  onSuccess: () => void;
}
...
const EditLandListingForm: React.FC<EditLandListingFormProps> = ({ land, onClose, onSuccess }) => {
  const { auth } = useAuth();
  ...
  useEffect(() => {
    if (land.marketInfo) {
      setFormData({
        askingPrice: land.marketInfo.askingPrice?.toString() || '',
        description: land.marketInfo.description || '',
        features: land.marketInfo.features || [],
        nearbyAmenities: land.marketInfo.nearbyAmenities || [],
        virtualTourUrl: land.marketInfo.virtualTourUrl || ''
      });
      
      // Set existing images
      setExistingImages(land.marketInfo.images || []);
    }
  }, [land]);
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (existingImages.length + images.length + files.length > 7) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (existingImages.length === 0 && images.length === 0) {
        throw new Error('Please keep at least one image or upload new ones');
      }
      ...
      // Submit to API
      await apiService.updateLandListing(land._id, submitData);
      
      onSuccess();
    } catch (error: any) {
      setError(error.message || 'Failed to update land listing');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">Edit Land Listing</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <span className="ml-2 font-medium text-green-600">Listed for Sale</span>
              </div>
            </div>
          </div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              ...
              Upload Photos (1-7 images total)
            </label>
...
export default EditLandListingForm;
